...
- Mise en page de la fiche (page 1, disposition du docx d'origine) : colonne gauche avec APPLICATIONS, MECHANICAL SPECIFICATIONS en liste, INTEGRATION, STORAGE, WARRANTY, COMPLIANCE AND REGULATORY et TEMPERATURE RISE CURVE (conditions d'essai + courbe) ; colonne droite avec le tableau ELECTRIC AND THERMAL SPECIFICATIONS à bandeau bleu marine (dont Peak power, Warm-up time à trois paliers « cible: durée », Electrical appliance). En bas, au même niveau : la courbe à gauche, le schéma du film (format paysage) et les notes à droite ; texte légal sur toute la largeur. Épaisseur, poids et garantie sont saisis dans les specs mais affichés dans la colonne gauche.
...
- Chaque produit porte deux images chargées depuis l'onglet Produit : le schéma du film, au format paysage, en bas à droite et la courbe de montée en température (cadre « TEMPERATURE RISE CURVE »). PNG ou JPG, redimensionnées à 1400 px de large max et encodées en JPEG (qualité 0,85) pour tenir dans le localStorage ; elles font partie de l'export / import JSON. Sans image, un cadre pointillé « à charger » apparaît dans l'aperçu (cadre vide dans le PDF). Les cotes en mm restent saisissables mais sont optionnelles.
