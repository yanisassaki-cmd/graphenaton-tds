...
- Mise en page de la fiche (page 1, disposition du docx d'origine) : colonne gauche avec APPLICATIONS, MECHANICAL SPECIFICATIONS en liste, INTEGRATION, STORAGE, WARRANTY, COMPLIANCE AND REGULATORY et TEMPERATURE RISE CURVE (conditions d'essai + courbe) ; colonne droite avec le tableau ELECTRIC AND THERMAL SPECIFICATIONS à bandeau bleu marine (dont Peak power, Warm-up time à trois paliers « cible: durée », Electrical appliance), le schéma du film et les notes ; texte légal en bas. Épaisseur, poids et garantie sont saisis dans les specs mais affichés dans la colonne gauche.
- Page 2 optionnelle par produit (case « Ajouter une page 2 ») : applications en cartes avec pictogramme (« Titre : description » par ligne, pictogramme choisi d'après le titre dans `src/page2.js` / `src/icons.js`), avec le paragraphe « Applications » en introduction. Contenus par défaut selon la version dans `page2Defaults()` (`src/schema.js`). L'éditeur prévient si la page 2 déborde.
